...
class Curl {
    ...
    **/
    function __construct() {
        ...
        $this->authUsername = Config::$authBasicUsername;
        $this->authPassword = Config::$authBasicPassword;
    }
    ...
    **/
    function request($method, $url, $vars = array()) {
        ...
        if (curl_errno($this->request)) {
            $response = curl_errno($this->request).' - '.curl_error($this->request);
        } 
  ...
  protected function http_login($username = '', $password = '', $type = 'any')
  {
    curl_setopt($this->request, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
    curl_setopt($this->request, CURLOPT_USERPWD,  $this->authUsername .":" . $this->authPassword);
